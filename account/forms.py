...
class UserCreationForm(forms.ModelForm):
    ...
    password2 = forms.CharField(
        label=" گذر واژه تکراری", widget=forms.PasswordInput
    )
...
def start_with_0(value):
    if value[0] != '0':
        raise forms.ValidationError('name should start with 0')
...
class CheckOtpForm(forms.Form):
    code = forms.CharField(widget=forms.TextInput(attrs={'claas': 'form-control'}),
                           validators=[validators.MaxLengthValidator(4)])


class AddressCreationForm(forms.ModelForm):
    # user elzami nist baraye ferestsdan
